...
type TicketQuery struct {
	...
	withCustomerID   *CustomerQuery
	withExpertID     *ExpertQuery
...
func (tq *TicketQuery) QueryCustomerID() *CustomerQuery {
...
// QueryExpertID chains the current query on the "expert_id" edge.
func (tq *TicketQuery) QueryExpertID() *ExpertQuery {
	query := (&ExpertClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TicketQuery) QueryDepartmentID() *DepartmentQuery {
...
func (tq *TicketQuery) Clone() *TicketQuery {
	...
	return &TicketQuery{
		...
		withCustomerID:   tq.withCustomerID.Clone(),
		withExpertID:     tq.withExpertID.Clone(),
		withDepartmentID: tq.withDepartmentID.Clone(),
...
func (tq *TicketQuery) WithCustomerID(opts ...func(*CustomerQuery)) *TicketQuery {
	query := (&CustomerClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withCustomerID = query
	return tq
}

// WithExpertID tells the query-builder to eager-load the nodes that are connected to
// the "expert_id" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TicketQuery) WithExpertID(opts ...func(*ExpertQuery)) *TicketQuery {
	query := (&ExpertClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withExpertID = query
	return tq
}

// WithDepartmentID tells the query-builder to eager-load the nodes that are connected to
// the "department_id" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TicketQuery) WithDepartmentID(opts ...func(*DepartmentQuery)) *TicketQuery {
	query := (&DepartmentClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withDepartmentID = query
	return tq
}
...
func (tq *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [3]bool{
			tq.withCustomerID != nil,
			tq.withExpertID != nil,
			tq.withDepartmentID != nil,
		}
	)
	if tq.withCustomerID != nil || tq.withExpertID != nil || tq.withDepartmentID != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withCustomerID; query != nil {
		if err := tq.loadCustomerID(ctx, query, nodes, nil,
			func(n *Ticket, e *Customer) { n.Edges.CustomerID = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withExpertID; query != nil {
		if err := tq.loadExpertID(ctx, query, nodes, nil,
			func(n *Ticket, e *Expert) { n.Edges.ExpertID = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withDepartmentID; query != nil {
		if err := tq.loadDepartmentID(ctx, query, nodes, nil,
			func(n *Ticket, e *Department) { n.Edges.DepartmentID = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TicketQuery) loadCustomerID(ctx context.Context, query *CustomerQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Customer)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Ticket)
	for i := range nodes {
		if nodes[i].customer_tickets == nil {
			continue
		}
		fk := *nodes[i].customer_tickets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "customer_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TicketQuery) loadExpertID(ctx context.Context, query *ExpertQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Expert)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Ticket)
	for i := range nodes {
		if nodes[i].expert_tickets == nil {
			continue
		}
		fk := *nodes[i].expert_tickets
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "expert_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TicketQuery) loadDepartmentID(ctx context.Context, query *DepartmentQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Department)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Ticket)
	for i := range nodes {
		if nodes[i].department_tickets == nil {
			continue
		}
		fk := *nodes[i].department_tickets
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "department_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
