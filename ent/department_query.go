...
func (dq *DepartmentQuery) QueryExperts() *ExpertQuery {
	query := (&ExpertClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withExperts: dq.withExperts.Clone(),
...
func (dq *DepartmentQuery) WithExperts(opts ...func(*ExpertQuery)) *DepartmentQuery {
	query := (&ExpertClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withExperts = query
	return dq
}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withExperts; query != nil {
		if err := dq.loadExperts(ctx, query, nodes,
			func(n *Department) { n.Edges.Experts = []*Expert{} },
			func(n *Department, e *Expert) { n.Edges.Experts = append(n.Edges.Experts, e) }); err != nil {
			return nil, err
		}
	}
...
func (dq *DepartmentQuery) loadExperts(ctx context.Context, query *ExpertQuery, nodes []*Department, init func(*Department), assign func(*Department, *Expert)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(department.ExpertsTable)
		s.Join(joinT).On(s.C(expert.FieldID), joinT.C(department.ExpertsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(department.ExpertsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(department.ExpertsPrimaryKey[0]))
