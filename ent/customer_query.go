...
func (cq *CustomerQuery) QueryTickets() *TicketQuery {
...
func (cq *CustomerQuery) WithTickets(opts ...func(*TicketQuery)) *CustomerQuery {
	query := (&TicketClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withTickets; query != nil {
		if err := cq.loadTickets(ctx, query, nodes,
			func(n *Customer) { n.Edges.Tickets = []*Ticket{} },
			func(n *Customer, e *Ticket) { n.Edges.Tickets = append(n.Edges.Tickets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CustomerQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Ticket)) error {
	...
	query.Where(predicate.Ticket(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.TicketsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.customer_tickets
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_tickets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_tickets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
